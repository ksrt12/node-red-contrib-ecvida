...
module.exports = {
    ...
    func: {
        ...
        /** @type {NodeCheckContext} */
        CheckContext(node, varsToCheck) {
            let should_update = false;
            /** @type {NodeContextData} */
            const flowContext = node.context().flow;
            /** @type {string} */
            const id = node.login_node.id;
            /** @type {typeof varsToCheck} */
            let storedId = flowContext.get(id) || {};
            Object.keys(varsToCheck).forEach(key => {
                if (varsToCheck[key] !== storedId[key]) {
                    storedId[key] = varsToCheck[key];
                    should_update = true;
                }
            });
            if (should_update) flowContext.set(id, storedId);
