...
module.exports = async ({ topic, cookies, SetError }) => {
    ...
    let balance_div, month, sum;

    while (typeof balance_div !== "object") {
        let document = await getHTML("https://lkabinet.online/accruals", topic, cookies, SetError);

        if (document) {
            month = document.querySelector("#placeForShowAccrual > div.page_title.accrualTitle > h2").textContent;
            sum = formatNumber(document.querySelector("#accrualPage > div.payment_right > div.accruals_total.accrualTotal > div.accr_head > div > div.prise").textContent);
